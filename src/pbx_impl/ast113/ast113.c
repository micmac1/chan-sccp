...
static int sccp_astwrap_indicate(PBX_CHANNEL_TYPE * ast, int ind, const void *data, size_t datalen)
{
	...
	*/
	...
	if (inband_if_receivechannel && c->rtp.audio.receiveChannelState != SCCP_RTP_STATUS_INACTIVE) {
		res = -1;
	}
...
static enum ast_rtp_glue_result __get_rtp_info(PBX_CHANNEL_TYPE * ast, PBX_RTP_TYPE ** rtp, sccp_rtp_type_t rtptype)
{
...
static int sccp_astwrap_setCallState(constChannelPtr channel, int state)
{
